...
class SimpleGaussianModel:
    ...
    def create_from_pcd(self, pcd, spatial_lr_scale: float = 1):
        ...
        fused_color = torch.tensor([1.0, 0.0, 0.0]).repeat(fused_color.shape[0], 1).cuda()
        ...
        scales = torch.log(torch.clamp_min(0.4*torch.rand((pcd.points.shape[0]))[..., None].repeat(1, 3), 0.0000001)).cuda()
    ...
    def distCUDA2(self, x):
        ...
        Compute pairwise squared distances between points in x.
         ...
         returns dist2: A tensor of shape (N, N) containing the pairwise squared distances.
        """
        # Compute the dot product between all points
        ...
        # dist2(i, j) = ||x_i - x_j||^2 = ||x_i||^2 + ||x_j||^2 - 2 * <x_i, x_j>
        dist2 = squared_norms.unsqueeze(0) + squared_norms.unsqueeze(1) - 2.0 * dot_product
        return dist2
